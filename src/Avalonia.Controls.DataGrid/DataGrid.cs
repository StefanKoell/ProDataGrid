...
internal
    ...
    partial class DataGrid : TemplatedControl
    {
        ...
        private Avalonia.Controls.DataGridFiltering.IFilteringModelInteraction _filteringModelInteraction;
        ...
        static DataGrid()
        {
            ...
            ColumnHeaderContextMenuProperty.Changed.AddClassHandler<DataGrid>((x, e) => x.OnColumnHeaderContextMenuChanged(e));
        ...
        /// <summary>
        /// Re-applies the grid-level column header context menu to realized headers that do not override it.
        /// </summary>
        private void OnColumnHeaderContextMenuChanged(AvaloniaPropertyChangedEventArgs e)
        {
            if (ColumnsInternal == null)
            {
                return;
            ...
            foreach (var column in ColumnsInternal.GetVisibleColumns())
            {
                if (column.HeaderContextMenu != null || !column.HasHeaderCell)
                {
                    continue;
                }

                column.ApplyHeaderContextMenu(column.HeaderCell);
            }
        }
        ...
        private void AttachExternalSubscriptions()
        {
            ...
            if (_boundColumns != null)
            {
                ...
                else if (!AreMaterializedBoundColumnsCurrent())
                {
            ...
            if (_columnDefinitionsSource != null)
            {
                ...
                else if (!AreMaterializedColumnDefinitionsCurrent())
                {
                    ApplyColumnDefinitionsSnapshot();
                }
                else
                {
                    RefreshMaterializedColumnDefinitions();
        ...
        private bool AreMaterializedBoundColumnsCurrent()
        {
            if (_boundColumns == null)
            {
                return true;
            ...
            List<DataGridColumn> expected;
            try
            {
                expected = _boundColumns.ToList();
            }
            catch
            {
                return false;
            ...
            var actual = ColumnsInternal
                .Where(column => column is not DataGridFillerColumn && !column.IsAutoGenerated)
                .ToList();
        ...
        private bool AreMaterializedColumnDefinitionsCurrent()
        {
            if (_columnDefinitionsSource == null)
            {
                return true;
            ...
            try
            {
                definitions = _columnDefinitionsSource.ToList();
            }
            catch
            {
                return false;
            ...
            var expectedDefinitions = new HashSet<DataGridColumnDefinition>(definitions.Count);
            var expectedColumns = new List<DataGridColumn>(definitions.Count);
            foreach (var definition in definitions)
            {
                if (definition == null || !_columnDefinitionMap.TryGetValue(definition, out var column) || column == null)
                {
                    return false;
                }

                if (!expectedDefinitions.Add(definition))
                {
                    return false;
                }
            ...
            if (_columnDefinitionMap.Count != expectedDefinitions.Count)
            {
                return false;
            }

            foreach (var definition in _columnDefinitionMap.Keys)
            {
                if (!expectedDefinitions.Contains(definition))
                {
                    return false;
                }
            }

            var actualColumns = ColumnsInternal
                .Where(column =>
                    column is not DataGridFillerColumn &&
                    !column.IsAutoGenerated &&
                    DataGridColumnMetadata.GetDefinition(column) != null)
                .ToList();
            ...
            var actualSet = new HashSet<DataGridColumn>(actualColumns);
            if (actualSet.Count != actualColumns.Count)
            {
                return false;
            }
            ...
            {
                if (!actualSet.Remove(expectedColumn))
                {
                    return false;
                }
            }
        ...
        private void SetFilteringModel(Avalonia.Controls.DataGridFiltering.IFilteringModel model, bool initializing = false)
        {
            ...
            if (_filteringModelInteraction != null)
            {
                WeakEventHandlerManager.Unsubscribe<Avalonia.Controls.DataGridFiltering.FilteringModelInteractionEventArgs, DataGrid>(
                    _filteringModelInteraction,
                    nameof(Avalonia.Controls.DataGridFiltering.IFilteringModelInteraction.ShowFilterFlyoutRequested),
                    FilteringModel_ShowFilterFlyoutRequested);
                _filteringModelInteraction = null;
            }
            ...
            if (_filteringModel is Avalonia.Controls.DataGridFiltering.IFilteringModelInteraction interaction)
            {
                _filteringModelInteraction = interaction;
                WeakEventHandlerManager.Subscribe<Avalonia.Controls.DataGridFiltering.IFilteringModelInteraction, Avalonia.Controls.DataGridFiltering.FilteringModelInteractionEventArgs, DataGrid>(
                    _filteringModelInteraction,
                    nameof(Avalonia.Controls.DataGridFiltering.IFilteringModelInteraction.ShowFilterFlyoutRequested),
                    FilteringModel_ShowFilterFlyoutRequested);
            }
        ...
        private void FilteringModel_ShowFilterFlyoutRequested(object? sender, Avalonia.Controls.DataGridFiltering.FilteringModelInteractionEventArgs e)
        {
            TryShowFilterFlyout(e.ColumnId);
        }
        ...
        /// <summary>
        /// Clears the active filter for the provided column.
        /// </summary>
        /// <param name="column">The column whose filter should be cleared.</param>
        /// <returns><c>true</c> if a filter was removed; otherwise <c>false</c>.</returns>
        public bool ClearFilter(DataGridColumn column)
        {
            if (column == null || _filteringModel == null || !ReferenceEquals(column.OwningGrid, this))
            {
                return false;
            }

            var descriptor = GetFilteringDescriptorForColumn(column);
            if (descriptor == null)
            {
                return false;
            }

            return _filteringModel.Remove(descriptor.ColumnId);
        }

        /// <summary>
        /// Clears the active filter for the provided column id.
        /// </summary>
        /// <param name="columnId">The column identifier used by the filtering model.</param>
        /// <returns><c>true</c> if a filter was removed; otherwise <c>false</c>.</returns>
        public bool ClearFilterByColumnId(object columnId)
        {
            if (columnId == null || _filteringModel == null)
            {
                return false;
            }

            return _filteringModel.Remove(columnId);
        }

        /// <summary>
        /// Attempts to display the filter flyout for the provided column id.
        /// </summary>
        /// <param name="columnId">The column identifier used by the filtering model.</param>
        /// <returns><c>true</c> if the flyout was shown; otherwise <c>false</c>.</returns>
        public bool TryShowFilterFlyout(object columnId)
        {
            if (columnId == null)
            {
                return false;
            ...
            var column = ResolveColumnById(columnId);
            return column?.TryShowFilterFlyout() == true;
        }

        private DataGridColumn ResolveColumnById(object columnId)
        {
            var columns = ColumnsInternal;
            if (columns == null)
            {
                return null;
            }
            ...
            foreach (var column in columns)
            {
                ...
                if (DataGridColumnMetadata.MatchesColumnId(column, columnId))
                {
                    return column;
                }
            }
